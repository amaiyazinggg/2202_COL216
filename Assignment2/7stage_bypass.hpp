#ifndef __7STAGE_BYPASS_HPP__
#define __7STAGE_BYPASS_HPP__
...
struct MIPS_Architecture
{
	...
	int op(std::string r1, std::string r2, std::string r3, std::function<int(int, int)> operation)
	{
		...
		if (choice == 0) registers[registerMap[r1]] = operation(registers[registerMap[r2]], registers[registerMap[r3]]);
		else storage[registerMap[r1]] = operation(storage[registerMap[r2]], storage[registerMap[r3]]);
		// PCnext = PCcurr + 1;
		return 0;
	}
	...
	int slt(std::string r1, std::string r2, std::string r3)
	{
		...
		if (choice == 0) registers[registerMap[r1]] = registers[registerMap[r2]] < registers[registerMap[r3]];
	...
	int lw(std::string r, std::string location, std::string unused1 = "")
	{
		...
		if (address < 0)
			return abs(address);
		if (choice == 0) registers[registerMap[r]] = data[address];
		else storage[registerMap[r]] = data[address];
		// PCnext = PCcurr + 1;
		return 0;
	}
	...
	*/
	...
	void run_RR(){
		...
		if (((RR -> instruct)[0] == "lw") || ((RR -> instruct)[0] == "sw")) EX = EX_mem;
		else EX = EX_reg;
		...
		if (EX -> occupied == 0){
			if ((RR -> instruct)[0] == "addi"){
				// std::cout << "hey1";
				std::string reg1 = (RR -> instruct)[1];
				std::string reg2 = (RR -> instruct)[2];
			...
			}else if (((RR -> instruct)[0] == "add") || ((RR -> instruct)[0] == "sub") || ((RR -> instruct)[0] == "mul") || ((RR -> instruct)[0] == "slt") ){
				// std::cout << "hey2";
				std::string reg1 = (RR -> instruct)[1];
				std::string reg2 = (RR -> instruct)[2];
				std::string reg3 = (RR -> instruct)[3];
			...
			}else if ((RR -> instruct)[0] == "lw"){
				// std::cout << "hey10";
				std::string reg2 = get_register((RR -> instruct)[2]);
				std::string reg1 = (RR -> instruct)[1];
				int new_address = locateAddress((RR -> instruct)[2]);
			...
			}else if (((RR -> instruct)[0] == "beq") || ((RR -> instruct)[0] == "bne")) {
				// std::cout << "hey4";
				std::string reg2 = (RR -> instruct)[1];
				std::string reg3 = (RR -> instruct)[2];
				
				if ((registerValueChange[reg2] == 0) && (registerValueChange[reg3] == 0)){
					choice = 1;
					instructions[(RR -> instruct)[0]](*this, (RR -> instruct)[1], (RR -> instruct)[2], (RR -> instruct)[3]);
	...
	void run_EX_reg(){
		...
		if (flag == 0){
			if (WB2 -> occupied == 0){
				...
				choice = 1;
				instructions[(EX_reg -> instruct)[0]](*this, (EX_reg -> instruct)[1], (EX_reg -> instruct)[2], (EX_reg -> instruct)[3]);

				
				std::string reg1 = (EX_reg-> instruct)[1];
	...
	void run_MEM2(){
		...
		if ((MEM2 -> instruct)[0] == "lw"){
			choice = 1;
			std::string reg1 = (MEM2-> instruct)[1];
	        registerValueChange[reg1] = 0;
	        instructions[(MEM2-> instruct)[0]](*this, (MEM2 -> instruct)[1], (MEM2 -> instruct)[2], (MEM2 -> instruct)[3]);
		}
	...
	void run_WB2(){
		...
		if (port == 0){
			...
			// std::cout << "old val is " << storage[registerMap[(WB2->instruct)[1]]];
			instructions[(WB2 -> instruct)[0]](*this, (WB2 -> instruct)[1], (WB2 -> instruct)[2], (WB2 -> instruct)[3]);
			// std::cout << "new val is " << storage[registerMap[(WB2->instruct)[1]]];
		}
		else{
			// std::cout << "hello";
			return;
